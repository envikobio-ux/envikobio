    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-[#F5F7FA] py-12 border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/blog" className="inline-flex items-center text-[#A1BA80] hover:text-[#4A90C2] transition-colors mb-6">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 bg-white text-[#A1BA80] px-8 py-3 rounded-lg font-semibold hover:bg-blue-50 transition-colors"
            >
            ...
            {BLOG_ARTICLES.filter(a => a.slug !== article.slug).slice(0, 3).map((related) => (
              <Link href={`/blog/${related.slug}`} key={related.slug} className="group">
                <div className="bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
                  <h3 className="text-base font-bold text-[#333333] mb-3 group-hover:text-[#A1BA80] transition-colors line-clamp-3" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    {related.title}
                  </h3>
                  <p className="text-sm text-[#999999] mb-3 line-clamp-2">{(related as any).excerpt}</p>
                  <span className="text-[#A1BA80] text-sm font-semibold flex items-center gap-1">
